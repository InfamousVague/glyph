...
2. **Format:** JSON Canvas 1.0 verbatim, so it opens in Obsidian and back; not a grammar of the app's own.
...
files use, verbatim (Matt chose interop over a grammar of the app's own). A canvas made in Ghost.md opens in
Obsidian; an Obsidian canvas pasted into Ghost.md is drawn the same. `src/app/canvas/jsonCanvas.ts` reads and writes
it.
...
**Colour** is the spec's: `"1"` to `"6"` are its six presets, whose looks each app chooses, or a hex colour.
Ghost.md paints the presets as the page's own hues - rose, ember, amber, moss, sea, violet, the ones a workspace wears
(`src/app/ink.css`) - and keeps a hex as it is.
...
A second one beside it, "How Ghost.md works" (`src/app/canvas/howCanvas.ts`), says what the app does in eight plain cards with
...
written into the note as the spec's JSON with the front matter kept (`src/app/canvas/jsonCanvas.ts` `withCanvas`),
saved the way typing is, so a canvas edited in Ghost.md still opens in Obsidian. A canvas shown where it cannot be written - a card
...
**The third slice draws lines.** The Draw a line tool turns the next two taps into a line, from the first card tapped
...
written, and a cross there takes the group off while the cards in it stay. Making a group round cards was left for a
later slice, and is still not built.

**The fifth slice adds more ways in** (choice 8). The Add a card tool is a sheet (`src/app/canvas/AddSheet.tsx`):
words, a note chosen by part of its title, or a web address (a bare one is given https). On a wide screen a note
dragged from the sidebar lands as a card where it is dropped (`src/app/notes/NoteTree.tsx` makes the rows
draggable). Pictures waited for the seventh slice, below. Adding by voice belongs to the capture and is not built.
...
(`src/app/core/images.ts`), so it syncs as they do - drawn to fill its card; a chart, a card of words that starts as a small
...
| file | what |
| --- | --- |
| `src/app/canvas/jsonCanvas.ts` | the format: read leniently, written exactly, and the note's body made from a canvas (`withCanvas`) |
| `src/app/canvas/edits.ts` | the cards and lines the view adds, and every edit to them, each a new canvas |
| `src/app/canvas/geometry.ts` | the box round the cards, where a line attaches, which sides two cards face by |
