...
- **Shared notes open here too.** Share links are `ghostmarkdown.com/read.html#…` (share/share.ts). The block also
...
- **The Caddyfile is shared.** `--caddy` writes only this domain's block, replacing it if it's there. It takes a backup, checks
