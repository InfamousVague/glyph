...
A finished recording may add to a note or make a new list, and nothing else: a table, a board, a lane or card move, a book, a chapter or a move of the recording is refused with "That command is not supported from a voice capture. Nothing changed." Those shapes are still read a phrase at a time by `src/app/capture/take.ts`, which only the voice test suite drives (docs/VOICE_TESTS.md).
...
`tools/host-tests` is a small cargo workspace that compiles the active `src-tauri/src/library/mod.rs`, the legacy migration `src-tauri/src/store.rs`, `src-tauri/src/llm/command.rs` and `src-tauri/src/llm/prompt.rs`, with the `note.rs` and `fsx.rs` they share, from the real source tree with `#[path]`—no copies—against only their direct dependencies. These modules contain no `tauri::` type; the harness stops compiling if that boundary changes.
