...
`src/app/plugins/registry.ts`. Nothing else in the app changes. The note's More sheet, the list swipe, the recorder,
the AI and Settings already ask the registry for whatever switched-on plugins offer.

## The manifest

This is `src/app/plugins/notion/manifest.ts`, with its reasons shortened:
...
All optional, all in `src/app/plugins/types.ts`:
...
it through `src/app/core/itemLinks.ts` `linkedLine(line, url, name)`: the words stay as they are and the item ends
with a mark, a link whose words are the plugin's lowercase name - `- [ ] Buy milk [notion](https://…)`. The editor
draws the mark as a small solid pill with the name on it, `unsentItems` skips marked items (and the older whole-words
form), and a run of the model keeps the mark's link by construction (`src/app/format/links.ts`): a link whose words
