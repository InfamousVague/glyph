...
fn main() {
    ...
    if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() {
        ...
        let bindings = bindings
            ...
            .clang_arg(format!("-I{}", ggml_include()))
    ...
    if let Some(dir) = shared_ggml() {
        config.define("WHISPER_USE_SYSTEM_GGML", "ON");
        config.define("ggml_DIR", dir.join("ggml"));
    }
    ...
    if cfg!(feature = "intel-sycl") {
        ...
        // llama-cpp-sys-2 links the one ggml, and every backend it built.
        if shared_ggml().is_none() {
            println!("cargo:rustc-link-lib=static=ggml");
            println!("cargo:rustc-link-lib=static=ggml-base");
            println!("cargo:rustc-link-lib=static=ggml-cpu");
        }
    }
    if shared_ggml().is_none() && (cfg!(target_os = "macos") || cfg!(feature = "openblas")) {
...
/// Glyph: the ggml CMake package llama-cpp-sys-2 installed, when it is a
/// dependency (`DEP_LLAMA_GGML_CMAKE_DIR`). Two ggml copies in one program link
/// under the same names and the linker mixes them; whisper.cpp builds against
/// llama.cpp's newer ggml unchanged.
fn shared_ggml() -> Option<PathBuf> {
    env::var_os("DEP_LLAMA_GGML_CMAKE_DIR").map(PathBuf::from)
}

/// The ggml headers bindgen reads: the shared ggml's, else the bundled ones.
fn ggml_include() -> String {
    match shared_ggml() {
        // <out>/lib/cmake -> <out>/include
        Some(dir) => dir.parent().and_then(|lib| lib.parent()).unwrap().join("include").display().to_string(),
        None => "./whisper.cpp/ggml/include".to_string(),
    }
}
