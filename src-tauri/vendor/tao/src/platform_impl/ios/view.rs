...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // Hand our +1 to the autorelease pool so the configuration outlives this
      // return. `as_ptr(&config)` let `config` drop before UIKit read it - a
      // use-after-free that crashed optimized builds on device in
      // -[UIApplication _connectUISceneFromFBSScene:transitionContext:].
      // (Backport of the upstream tao 0.36.0 fix; sole local change.)
