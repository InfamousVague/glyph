//! A model file, whichever engine reads it: what one must be, whether it is on
//! this device, and the verified, resumable download that puts it there.
//! whisper's catalogue (`whisper::model`) and the formatter's (`llm::model`)
//! are lists of `ModelSpec`s, and both go through here.
//!
...
//! trusts wrongly.
//!
//! It takes no Tauri types, like the engines it serves; the one piece of the
//! app it leans on is the async runtime Tauri starts, to sleep between tries
//! off the task that is downloading. The Tauri half - where models live, the
//! progress event, one download at a time - is `model_downloads`.
...
/// rule both catalogues follow (`whisper::model::MIRRORS`, and the formatting
...
mod tests {
    use super::*;
    use crate::whisper::model::{ACTIVE, REFINE};
