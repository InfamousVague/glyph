 ...
 *
 * Discard undoes what drafts wrote (`undoDraft`): a continued note gets its text back, a new note goes. The screen
...
export class TakeWriter {
   ...
   */
  baseBody: Promise<string> | null = null;
  ...
  /** The take is written into `note` from here, or into a new note with a fresh id when it is null. */
  ...
  rebase(note: Note): void {
    ...
    this.target = note;
    this.baseBody = Promise.resolve(note.body);
    this.host.targetChanged(note);
  }
  ...
  async compose(markdown: string): Promise<string> {
    ...
    this.baseBody ??= getNote(continued.id)
      .catch(() => null)
      .then((stored) => stored?.body ?? continued.body);
    return appendBody(await this.baseBody, markdown);
  }
  ...
  async undoDraft(): Promise<void> {
    ...
    const continued = this.target;
    if (continued) await this.persist(continued.id, (await this.baseBody) ?? continued.body, continued.source);
  ...
  updateNote(id: string, change: (body: string) => string | null): Promise<string | null> {
    return this.queue(async () => {
      ...
      if (continued?.id === id) {
        ...
        const base = await (this.baseBody ??= Promise.resolve(fresh.body));
        const next = change(base);
        if (next === null || next === base) return null;
        this.baseBody = Promise.resolve(next);
