...
import { countsLine, diagnosticsLine, EMPTY_DIAGNOSTICS, soundsSilent } from './diagnostics.ts';
...
describe('what the pipeline has done, in a line', () => {
  it('says how much was heard, at what rate, and what came of it, one or many', () => {
    expect(countsLine({ ...EMPTY_DIAGNOSTICS, heardSamples: 131_200, deviceRate: 48_000, partials: 3, segments: 1 })).toBe('heard 8.2 s at 48 kHz · 3 guesses · 1 phrase');
    expect(countsLine({ ...EMPTY_DIAGNOSTICS, partials: 1, segments: 2 })).toBe('heard 0.0 s · 1 guess · 2 phrases');
  });

  it('ends with the errors and the last of them', () => {
    expect(countsLine({ ...EMPTY_DIAGNOSTICS, errors: 2, lastError: 'capture_push rejected' })).toBe('heard 0.0 s · 0 guesses · 0 phrases · 2 errors: capture_push rejected');
    expect(countsLine({ ...EMPTY_DIAGNOSTICS, errors: 1, lastError: null })).toMatch(/1 error: $/);
  });
