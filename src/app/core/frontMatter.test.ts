...
describe('where the front matter ends in the body', () => {
  it('is after the closing fence, or the start where there is none', () => {
    expect(frontMatterOffset('---\ntitle: "A"\n---\n# A\n')).toBe(19);
    expect(frontMatterOffset('# A\n')).toBe(0);
    expect(frontMatterOffset('---\ntitle: "A"\n')).toBe(0);
    expect(frontMatterOffset('---\n---')).toBe(7);
  });

  // Until 2026-09-25 an AI run on the note (ai/start.ts) had its own reader that took any line inside, so a rewrite
  // kept a rule with words under it from the model while the editor showed those words.
  it('is the start of a note that opens with a rule and words', () => {
    expect(frontMatterOffset('---\nSome words here\n---\n# A\n')).toBe(0);
    expect(frontMatterOffset(keys(39).join('\n'))).toBe(0);
  });
});
