 ...
 * from a list. The list this replaced named seventeen keys, three of them no
 * longer written by anything, and had fallen twenty-five behind:
...
export async function forgetAccount(): Promise<void> {
  if (accountState().session) {
    try {
      ...
      throw new Error(`Could not sign out, so nothing was reset: ${failureText(failure)}`, { cause: failure });
    }
  }
  await syncSettled();
