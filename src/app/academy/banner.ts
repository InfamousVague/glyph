...
/**
 * Whether the home page offers Glyph Academy: the Academy card among the
 * home page's notices (notes/Notices.tsx `AcademyCard`), offered the way an
 * update is (Matt: "the academy page should show up on the home screen kinda
 * like the update banner for new users as a call to action banner" - hence
 * the file's name, and the names App.tsx knows it by).
 *
 * It is for someone who has not started: the moment a lesson is passed
 * (academy/lessons.ts keeps that), the card has done its job and goes. A
...
export function academyBannerDue(): boolean {
  return readProgress().size === 0 && !dismissed.is();
}
