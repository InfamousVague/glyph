...
describe('the screen opening shows', () => {
  it('says what it is doing, ticks each step off, asks for the update check at once, and hands over when done', () => {
    ...
    const onDone = vi.fn();
    const first = updates();
    host = show(<LaunchScreen loading notes={0} updates={first} sync={sync} onDone={onDone} />);
    ...
    rerender(<LaunchScreen loading={false} notes={12} updates={updates({ checking: true })} sync={{ ...sync, phase: 'syncing' }} onDone={onDone} />);
    expect(text()).toContain('12 notes');
    expect(text()).toContain('Checking for updates');
    expect(text()).toContain('Syncing your devices');
    rerender(<LaunchScreen loading={false} notes={12} updates={updates({ lastChecked: 1 })} sync={{ ...sync, phase: 'idle' }} onDone={onDone} />);
  ...
  it('shows no update line where no check runs, and doesn’t wait for one', () => {
    ...
    native = true;
    const onDone = vi.fn();
    host = show(<LaunchScreen loading={false} notes={3} updates={updates()} sync={sync} onDone={onDone} />);
...
describe('the ghost on the screen', () => {
  it('draws two eyes over the picture, and turns them toward the bar as it goes round', () => {
    ...
    host = show(<LaunchScreen loading notes={0} updates={updates()} sync={sync} onDone={() => {}} />);
