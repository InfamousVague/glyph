 ...
 *
 * A block widget is drawn after the note is laid out - a board once its cards are measured (editor/boards.ts), a
 * diagram once mermaid has drawn it (editor/mermaid.ts) - and until then the editor counts it at the widget's
 ...
 *
 * Kept under one key per kind of block, newest last, the oldest let go past a cap, and written half a second after
 * the last change so a board settling through several sizes is one write. Each kind keeps the key, the cap and the
 * rounding it has always had: a build rolled back over the air reads what the newer one wrote.
 */
...
/**
 * A memory kept in storage under `key`, holding at most `kept` blocks, each height passed through `round` first.
 * Storage is read the first time the memory is asked, not when it is made.
 */
export function heightMemory(key: string, kept: number, round: (height: number) => number): HeightMemory {
  ...
  const all = (): Map<string, number> => {
    ...
    heights = new Map();
    // No storage: blocks are guessed at until they are drawn.
    for (const entry of readStored<unknown[]>(key, [], (value) => (Array.isArray(value) ? value : null))) {
