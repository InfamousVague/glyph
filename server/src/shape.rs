...
//!
//! `model.rs` owns the conversation with Ollama and `main.rs` owns the wire.
...
pub const TITLE_WORDS: usize = 8;
/// The contract's ceiling on a section heading, in words.
pub const HEADING_WORDS: usize = 5;

/// Generous caps on how many of each thing one response may carry.
///
/// Not the quality rule - that is the grammar's `maxItems` in `model.rs`,
